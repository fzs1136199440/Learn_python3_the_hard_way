...
| `truncate([size])`                                           | 它在`open()`打开一个文件之后使用，用于从文件对象首行首字符开始截断，截断的文件长度为从首字符开始到第size个字节，意思是这个文件对象的文件长度现在只有”size个字节”了，“第size个字节”之后的内容就被删除了，不存在了。如果不提供size参数，就从首字符截断到当前位置（**有疑惑，待定......**） |
| `write(str)`                                                 | 在`open()`之后，在文件里写入字符串。                         |
...
| `file.readline([size])`       | "inside `readline()`is code that scans each byte of the file until it finds `\n`character, then stops reading the file to return what it found so far."这个函数以文件开头为起点，如果不提供参数`size`，则每次读取一行内容，包括换行符`\n`；如果参数`size`存在且非负，则返回`size`个字节。 |
| `return`                      | 用于函数中，用来返回指定的值。                               |
